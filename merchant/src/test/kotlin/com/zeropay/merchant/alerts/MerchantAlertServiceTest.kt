...
class MerchantAlertServiceTest {
    ...
    @Test
    fun `test send alert with valid merchant`() = runBlocking {
    ...
    @Test
    fun `test send alert to merchant without webhook configured`() = runBlocking {
        val result = alertService.sendAlert(
    ...
    @Test
    fun `test critical priority uses all channels`() = runBlocking {
        val result = alertService.sendAlert(
            merchantId = "merchant-123",
    ...
    @Test
    fun `test low priority uses database only`() = runBlocking {
        val result = alertService.sendAlert(
            merchantId = "merchant-123",
    ...
    @Test
    fun `test alert history is recorded`() = runBlocking {
    ...
    @Test
    fun `test alert history respects max size`() = runBlocking {
    ...
    @Test
    fun `test pending alerts are queued`() = runBlocking {
    ...
    @Test
    fun `test successful delivery removes from queue`() = runBlocking {
    ...
    @Test
    fun `test different priority levels`() = runBlocking {
    ...
    @Test
    fun `test different alert types`() = runBlocking {
    ...
    @Test
    fun `test webhook payload contains required fields`() = runBlocking {
    ...
    @Test
    fun `test alert service handles errors gracefully`() = runBlocking {
    ...
    @Test
    fun `test multiple merchants can receive alerts independently`() = runBlocking {
        val merchant1Alert = testAlert.copy(userId = "user-001")
        val merchant2Alert = testAlert.copy(userId = "user-002")
