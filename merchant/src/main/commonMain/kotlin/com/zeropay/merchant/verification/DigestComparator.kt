...
class DigestComparator {
    ...
    fun compare(digest1: ByteArray, digest2: ByteArray): Boolean {
        ...
        try {
            ...
            if (digest1.size != EXPECTED_DIGEST_SIZE || digest2.size != EXPECTED_DIGEST_SIZE) {
                Log.w(TAG, "Invalid digest size: ${digest1.size} vs ${digest2.size}, expected: $EXPECTED_DIGEST_SIZE")
            ...
            if (executionTime > COMPARISON_TIMEOUT_MS) {
                Log.w(TAG, "Digest comparison exceeded timeout: ${executionTime}ms")
                throw SecurityException("Digest comparison timeout exceeded")
            }
            
            Log.d(TAG, "Digest comparison completed in ${executionTime}ms: ${if (result) "MATCH" else "NO MATCH"}")
            
            return result
            
        } catch (e: Exception) {
            Log.e(TAG, "Digest comparison error", e)
            throw e
        } finally {
    ...
    fun batchCompare(
        ...
        if (submittedDigests.keys != enrolledDigests.keys) {
            Log.w(TAG, "Factor mismatch in batch compare")
            return false
        }
