...
class FraudDetectorComplete {
    ...
    private val userTransactions = ConcurrentHashMap<String, MutableList<TransactionRecord>>()
    
    // Behavioral profiles
    private val userBehaviorProfiles = ConcurrentHashMap<String, BehavioralProfile>()
    
    // Known malicious entities
    private val blacklistedIPs = ConcurrentHashMap<String, BlacklistEntry>()
    private val blacklistedDevices = ConcurrentHashMap<String, BlacklistEntry>()
    ...
    suspend fun checkFraud(
        ...
        Log.i(TAG, "Fraud check result for $userId: score=$finalScore, level=$riskLevel")
        if (reasons.isNotEmpty()) {
            Log.d(TAG, "  Reasons: ${reasons.joinToString("; ")}")
        }
    ...
    private fun checkVelocity(userId: String, now: Long): RiskResult {
        ...
        if (attemptsLastMinute > MAX_ATTEMPTS_PER_MINUTE) {
            ...
            Log.w(TAG, "High velocity detected: $userId - $attemptsLastMinute/min")
        }
    ...
    private fun checkGeolocationAnomalies(
        ...
        if (locationList.isNotEmpty()) {
            ...
            Log.d(TAG, "Location change: ${distanceKm.roundToInt()} km in ${timeDiffHours.round(2)} hours = ${speedKmH.roundToInt()} km/h")
            ...
            if (speedKmH > SUSPICIOUS_TRAVEL_SPEED_KMH) {
                ...
                Log.w(TAG, "Impossible travel detected: $userId - $speedKmH km/h")
    ...
    private fun checkDeviceFingerprint(
        ...
        if (!userDevices.contains(deviceFingerprint) && userDevices.isNotEmpty()) {
            ...
            Log.i(TAG, "New device for user $userId: $deviceFingerprint")
        }
    ...
    private fun checkBehavioralPatterns(
        ...
        if (behavioralData.factorCompletionTimeMs < MIN_FACTOR_COMPLETION_MS) {
            ...
            Log.w(TAG, "Bot-like behavior detected: $userId")
        }
        ...
        if (profile.hasHistory()) {
            ...
            if (deviation > 0.7) {
                ...
                Log.w(TAG, "Behavioral anomaly detected: $userId - ${deviation}x deviation")
    ...
    private fun checkIPReputation(
        ...
        if (ipAttemptsLastHour > MAX_ATTEMPTS_PER_HOUR) {
            ...
            Log.w(TAG, "Suspicious IP activity: $ipAddress - $ipAttemptsLastHour/hour")
        }
    ...
    private fun checkTransactionAnomalies(
        ...
        if (transactionList.isNotEmpty()) {
            ...
            if (transactionAmount > avgAmount * HIGH_RISK_AMOUNT_MULTIPLIER) {
                ...
                Log.w(TAG, "Extremely high transaction: $userId - $transactionAmount vs avg $avgAmount")
    ...
    suspend fun clearFraudReport(
        ...
        if (deviceFingerprint != null) {
            ...
            Log.i(TAG, "Device whitelist: $deviceFingerprint")
        }
        
        if (ipAddress != null) {
            blacklistedIPs.remove(ipAddress)
            Log.i(TAG, "IP whitelisted: $ipAddress")
        }
    }
